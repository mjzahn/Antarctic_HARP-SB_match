...
harp <- read_excel("Antarc_EA_01_df100_new.xls")

```


## Identify Sonobuoys of interest

Identify which sonobuoys had calls that we expect to arrive at HARP so I can look at those data in Raven
...
## filter whales within 50 km of sonobuoy (good localizations)
...
```{r SB_IDs}
## load SB ID info
SB_all_ID <- read_csv("sonobuoy_IDs.csv")
SB_all_ID$Latitude <- round(SB_all_ID$Latitude, 2)
SB_all_ID$Longitude <- round(SB_all_ID$Longitude, 2)

## round SB detection lat/lon data to match SB_ID lat/lon data
SB_filtered_latlon <- round(SB_filtered[,4:5], 2)
## change column names to match SB_ID dataset
SB_filtered_latlon <- SB_filtered_latlon %>% 
  rename(Latitude = 'BuoyLatitude', Longitude = 'BuoyLongitude')

## merge dataframes to find matching latlon values for detections
dets_merged <- left_join(SB_filtered_latlon, SB_all_ID, 
                         by = c('Latitude', 'Longitude'))

length(unique(dets_merged$BuoyID)) # number of SB to analyze

## extract unique BuoyIDs and retain other column data
SB_ID_final <- distinct(dets_merged, BuoyID, .keep_all = TRUE)

write.csv(SB_ID_final, "SB_ID_filtered.csv") # SB to be analyzed in Raven

```


## Determine timestamps when we expect SB-detected calls to arrive at HARP
...
SB_final <- SB_filtered[,c("Species", "Channel", "DateTime", "SB_whaletime", 
...
SB_final$Channel[which(SB_final$Channel == 2)] <- 3
SB_final$Channel[which(SB_final$Channel == 1)] <- 2
SB_final$Channel[which(SB_final$Channel == 0)] <- 1

## format date from excel numerical date/time to POSIXct in UTC
# harp$DateTime <- as.Date(harp$Time, origin = "1899-12-30")
harp$DateTime <- as.POSIXct(harp$Time, origin = "1899-12-30", tz = 'UTC',
                                   format = "%a %b %d %Y %H:%M:%OS")        
# harp$DateTime <- with_tz(harp$DateTime, tz = "UTC")

#### match 'SB_harptime' with harp detection times for each call type-------------------------

timefudge <- 10 # find match within 10 s = approx 15 km error
...
## extract matches
colnames(SB_final) <- c("Call", "Channel", "SBtime", "SB_whaletime", "SB_harptime", "SB_whale_Dist_m",
                        "harp_whale_Dist_m", "harpcall_ID", "match", "match_harptime")
SB_harp_matches <- SB_final %>% filter(match == 1) %>% 
  select(Call, Channel, SBtime, SB_whaletime, SB_harptime, match_harptime, SB_whale_Dist_m, harp_whale_Dist_m)
