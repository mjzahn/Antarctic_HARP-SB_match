...
SB <- read_csv("DIFAR_Localisation_WithErrorEstimates.csv")
harp <- read_csv("Antarc_EA_review_fixedFin.csv")
...
SB <- SB[,c("UID", "UTC", "BuoyLatitude", "BuoyLongitude", "Species", 
...
SB_filtered$DateTime <- as.POSIXct(SB_filtered$DateTime,origin="1899-12-30", tz = 'UTC',
...
SB_final <- SB_filtered[,c("Species", "DateTime", "SB_harptime", "SB_whaletime")]
...
SB_final$harpcall <- rep(NA, nrow(SB_final))
SB_final$match <- rep(NA, nrow(SB_final))
SB_final$match_harptime <- rep(as.POSIXct(NA, tz="UTC"),nrow(SB_final))

for (i in 1:nrow(SB_final)){
  calltype <- as.character(SB_final[i,"Species"]) # determine SB call type
  harp_working <- harp[which(harp$Call == calltype),] # all harp data for call type
  harp_times <- harp$DateTime[which(harp$Call == calltype)] # harp times for same call type
  ...
  if (any(timeDiff < timefudge) == FALSE){ # if no timeDiff < 10 s
    ...
    match_harptime <- harp_times[which(timeDiff %in% min(timeDiff))][1] # take minimum time diff
    SB_final$harpcall[i] <- harp_working$Call[which(timeDiff %in% min(timeDiff))][1]
...
## extract matches
colnames(SB_final) <- c("Call", "SBtime", "SB_harptime", 
                        "SB_whaletime","harpcall_ID", "match", "match_harptime")
SB_harp_matches <- SB_final %>% filter(match == 1) %>% 
  select(Call, SBtime, SB_harptime, SB_whaletime, match_harptime)
